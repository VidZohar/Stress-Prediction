...
class DataManager:
    ...
    def get_subject_path(self, subject):
        ...
        path = os.path.join(DataManager.ROOT_PATH, 'S' + str(subject) + DataManager.FILE_EXT)
...
#              "of ", str(window_size) + "." )        
    ...
    def compute_features(self, subjects=SUBJECTS, data=BASELINE_DATA, window_size=42000, window_shift=175):
        ...
        pickle.dump(DataManager.FEATURES, open("features.pkl", "wb" ))
    ...
    def compute_features_stress(self, subjects=SUBJECTS, data=STRESS_DATA, window_size=42000, window_shift=175):
        ...
        pickle.dump( DataManager.STRESS_FEATURES, open( "features_stress.pkl", "wb" ) )
    ...
    def scale_SVM_data(self, X_train, X_test, y_train, y_test):
        (X_train, X_test, y_train, y_test) = self.get_train_and_test_data()
        sc = StandardScaler()
    ...
    def create_SVM(self):
        (X_train, X_test, y_train, y_test) = self.get_train_and_test_data()
        (X_train, X_test, y_train, y_test) = \
            self.scale_SVM_data(X_train, X_test, y_train, y_test)
        # Train SVM with 'linear' or non-linear 'rbf' kernel
        classifier = SVC(kernel = 'rbf', random_state = 0)
        classifier.fit(X_train, y_train)

        return (X_train, X_test, y_train, y_test, classifier)

    def get_SVM_results(self, X_train, X_test, y_train, y_test, classifier):
        y_pred = classifier.predict(X_test)
        print("_________________________________________________________________")
        print('\nClassification report from classifier.predict with test data')
        print(classification_report(y_test, y_pred))
        print("_________________________________________________________________")
        print('\nConfusion matrix from classifier.predict with test data')
        print(confusion_matrix(y_test, y_pred))
